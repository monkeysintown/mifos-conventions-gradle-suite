...
import static org.mifos.conventions.gradle.template.MifosGradleTemplateConstants.MIFOS_TEMPLATE_GIT_PATTERN;
...
public abstract class MifosGradleTemplateIndexCreateTask extends DefaultTask {
            ...
            if (path.endsWith(MIFOS_TEMPLATE_FILE_EXTENSION)
                    && lines.map(line -> {
                                ...
                                if (details.getPath().endsWith(MIFOS_TEMPLATE_FILE_EXTENSION)) {
                                    var matcher = Pattern.compile(MIFOS_TEMPLATE_DEFAULT_PATTERN)
                                            .matcher(line);
    ...
    private String pathParameters(
            String path,
            MifosGradleTemplateIndex.TemplateGroup.TemplateGroupBuilder groupBuilder,
            Set<String> paramNames) {
        var matches = new HashMap<String, String>();
        var matcher = Pattern.compile(MIFOS_TEMPLATE_DEFAULT_PATH_PATTERN).matcher(path);
        ...
        if (path.endsWith(MIFOS_TEMPLATE_FILE_EXTENSION)) {
            path = path.substring(0, path.lastIndexOf(MIFOS_TEMPLATE_FILE_EXTENSION));
        }

        path = path.replaceAll(MIFOS_TEMPLATE_GIT_PATTERN, "/.git/");
    ...
    private MifosGradleTemplateIndex merge(MifosGradleTemplateIndex oldIdx, MifosGradleTemplateIndex newIdx) {
        ...
        for (var group : resultIdx.getGroups()) {
            if (group.getParameterGroups() == null) {
                group.setParameterGroups(new ArrayList<>());
            }
